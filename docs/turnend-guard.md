...
Once that beacon is stale, the pull guard stays healthy only when the current process belongs to the Claude session that owns the home lock, the durable recovery marker is exactly `pending:handling:<generation>`, and neither an auto-arm failure marker nor a failed epoch exists.
That recovery state begins when the drain presents a watcher-delivered recovery generation and ends when handling acknowledgement changes it away from `pending:handling`, so a handling turn may cross the 300-second beacon grace without being mislabeled as watcher downtime while its next watcher is not due until Stop.
An absent, malformed, downtime, or acknowledged recovery marker remains loud after grace, as does a competing session or any recorded Claude auto-arm failure.
Cursor shares the broad between-turn `autoarm` model but uses its separate stop-hook park and does not inherit Claude's pending-handling tolerance.
...
`tests/fm-guard-stale-banner.test.sh` covers the pull-guard predicate, including the persistent-model fresh-leftover-beacon negative control, the auto-arm model's healthy fresh-beacon-without-a-watcher case, the exact Claude pending-handling 299-second/301-second boundary, completed-handling and failure-marker alarms, durable queued-wake independence, Cursor isolation, the five runtime backends, and the extension model's live-watcher path, ownership-qualified fresh hand-off, held-lock failures, independently broken ownership signals, stale-beacon alarm, and Pi and pi-signed harness routing.
